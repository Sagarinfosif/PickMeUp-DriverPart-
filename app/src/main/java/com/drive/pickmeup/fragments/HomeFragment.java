...
public class HomeFragment extends Fragment implements View.OnClickListener, OnMapReadyCallback {
    ...
    private TextView nav_my_vehicle, nav_settings, nav_drive_partner, nav_inbox, nav_account, side_name;
    private AppCompatImageView imgSide;
   private Animation animation;
    private int data = 0;
    private int ride = 0;
    private int on = 1;
    private RegisterPojo loginDetail;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        loginDetail = App.getSharedpref().getModel(AppConstants.LOGIN_DETAIL, RegisterPojo.class);
        ids();
        setup();
        performActions();
        openLayout();
...
//        }
    ...
    private void setDataOnSideDrawer() {
        side_name.setText(loginDetail.getDetails().getUsername());

        if (!loginDetail.getDetails().getImage().equals("")) {
            Glide.with(getContext()).load(loginDetail.getDetails().getImage()).into(userImage);
            Glide.with(getContext()).load(loginDetail.getDetails().getImage()).into(user_pic);
        }
    ...
    private void setup() {
        ...
        imgSide.setBackgroundResource(R.drawable.ic_three_lines);
    ...
    private void ids() {
        ...
        side_name = view.findViewById(R.id.side_userName);
        ...
        imgSide = view.findViewById(R.id.back_topbar);
        //floating button
